...
func (m *DockerManager) ContainerCreate(ctx context.Context, h model.Host, spec model.ContainerSpec) (string, error) {
	cli, err := m.Client(h)
	if err != nil {
		return "", err
	}
...
func buildPortBindings(mappings []model.PortMapping) (nat.PortSet, nat.PortMap, error) {
	if len(mappings) == 0 {
		return nil, nil, nil
	}
