...
func TestServiceInstanceValidation(t *testing.T) {
	...
	if err := Validate(model.ServiceInstance{ServiceTypeID: "type-1", HostID: "host-1", Name: "Game-1"}); err == nil {
		t.Fatal("expected lowercase name validation error")
	}
	if err := Validate(model.ServiceInstance{ServiceTypeID: "type-1", HostID: "host-1", Name: "game-1"}); err == nil {
		t.Fatal("expected image required validation error")
	}
	if err := Validate(model.ServiceInstance{ServiceTypeID: "type-1", HostID: "host-1", Name: "game-1", Image: "game:v1", Params: []model.ServiceParam{{Flag: "server-id", Value: "1"}}}); err == nil {
		t.Fatal("expected flag validation error")
	}
}

func seedInstance(t *testing.T, a *InstanceService) model.ServiceInstance {
	t.Helper()
	item := NewServiceInstance("type-1", "host-1", "game-1", "game:v1", "", "", "", "", nil)
